...
class Console
{
    ...
    public function reader()
    {
        ...
        if (strpos($this->comand, 'mcquery server --port:') === 0) {
            $this->render = true;
            $controller_s = str_replace("mcquery server --port:", "", $this->comand);
            $controller_s = str_replace(" ", "", $controller_s);

            if (is_numeric($controller_s)) {
                if (strlen($controller_s) == 4) {
                    echo "\033[0;32mServidor de desenvolvimento ativo:\033[0m http://127.0.0.1:$controller_s" . PHP_EOL;
                    shell_exec("php -S 127.0.0.1:$controller_s App/Server.php");
                } else {
